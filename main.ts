import { App, Modal, Notice, Plugin, PluginSettingTab, Setting, ButtonComponent, TextComponent, MarkdownPostProcessorContext} from 'obsidian';
...
export default class MyPlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			// Can use this to extract the database of tags and projects from OF
			id: 'project-from-note',
			name: 'Create project from current note',
			 callback: () => {
				...
				if (activeFile){
					...
					new Notice("No active file to create a project from.")
				}
			},
		});
...
class InboxModal extends Modal {
	...
	onOpen() {
        ...
        let taskNameField = new TextComponent(contentEl)
		...
		let addTaskButton = new ButtonComponent(contentEl)
			...
			// On Click callback
			.onClick(() => {
				addTask(taskNameField);
			});
    // Focus on the text field
    taskNameField.inputEl.focus();
	taskNameField.inputEl.addEventListener('keypress', function (keypressed) {
...
function addTask(taskNameField: TextComponent){
	// Add quotes so it can be used in AppleScript
	var taskName = "\"" + taskNameField.getValue() + "\""
